...
// 1T单片机@11.0592MHz主频下，只要这个值超过10，那么延时时间就超过了4.7uS
...
void I2C_Init(){
	P0M0 |= 0x03;
	P0M1 |= 0x03;	// 将I2C_SCL和I2C_SDA设置成开漏
...
void CheckI2CSlaveValidity(){
	u8 i = 0;
	Uart_String("Start checking slaves!\r\n");
	I2C_Init();
	while(1){
		bit result;
		I2C_Start();
		...
		if(result == 0){
			Uart_String("Slave ");
			Uart_Hex(i);
			Uart_String(" is valid.\r\n");
		}
		I2C_Stop();
		i++;
		if(i == 0)break;
	}
	Uart_String("Finished checking slaves!\r\n");
}
